...
//----------------------------------------------------------
@ $db=new mysqli('localhost', 'root', 'rootadmin', 'gwac_obs_log');
...
$obsmap = "idlfile/Mini-GWAC_observing_area_map_".$filedate.".dat";
$fp = fopen($obsmap,"rb");
$ra = array();
$dec = array();
$data = array(array());
...
if(!get_magic_quotes_gpc())	{
	$ID=addslashes($ID);
	$GRB=addslashes($lines);}
$query= "insert into GCN_file values ('".$ID."','".$GRB."') ";
$result= $db->query($query);
if($result){
	$db->affected_rows;
	echo $GRB.":  Data inserted into database.";
	echo "<br>";
	 }
else {
...
$query2= "insert into ".$GRB." values ('".$ID."','".$name." ','".$GRB."'
,'".$ITEM_NOTE."','".$ITEM_NOTE2."','".$ITEM_NOTE3."','".$GCN."') ";
$result2= $db->query($query2);
if($result2){
	$db->affected_rows;
	echo $GRB.":  Data table created.";
	echo "<br>";	
	 }
else {
	echo "An error has occurred. The table was not created.";
	echo "<br>";	
	}	
//-------------------------------------------------------------------	
	 }
